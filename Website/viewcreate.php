...
function km_t_nb($con){
	...
	while ($record = mysqli_fetch_assoc($query)) {
		mysqli_query($con,"INSERT INTO `epiz_23212043_merapi`.`kmeans_training_naivebayes` (`No`, `Total_Pengungsi`, `Kebutuhan_Mendesak`, `Medis`, `Psikolog_Rohani`, `Teknis`, `Prioritas`) VALUES 
        ('".$record['Data']."', '".$record['Total_Pengungsi']."',  '".$record['Kebutuhan_Mendesak']."',  '".$record['Medis']."',  '".$record['Psikolog_Rohani']."',  '".$record['Teknis']."',  '".$record['Prioritas']."');");
	}
}
?>