...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `kmeans_training_naivebayes`  AS  select `mentah`.`No` AS `No`,`mentah`.`Update_Terakhir` AS `Update_Terakhir`,`mentah`.`Nama_Posko` AS `Nama_Posko`,`mentah`.`Dusun` AS `Dusun`,`mentah`.`Desa` AS `Desa`,`mentah`.`Kecamatan` AS `Kecamatan`,`mentah`.`Kabupaten` AS `Kabupaten`,`mentah`.`Asal_Pengungsi` AS `Asal_Pengungsi`,`mentah`.`Total_Pengungsi` AS `Total_Pengungsi`,`mentah`.`Kebutuhan_Mendesak` AS `Kebutuhan_Mendesak`,`mentah`.`Medis` AS `Medis`,`mentah`.`Psikolog_Rohani` AS `Psikolog_Rohani`,`mentah`.`Teknis` AS `Teknis`,`mentah`.`Prioritas` AS `Prioritas` from (`mentah` join `union_naivebayes` on((`mentah`.`No` = `union_naivebayes`.`Data`))) ;
